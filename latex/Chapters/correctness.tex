...
The graph structure is directed graph in which the syntax and semantics are discussed in detail in sections \ref{sec:statechartsyn}, \ref{sec:statechartsem}. Our tool constructs executable code from on the diagram constructs. In order to show correctness we will demonstrate that our code follows figure \ref{fig:correctness_graph1}. Figure \ref{fig:correctness_graph1} essentially says that an evaluation of each expression given by $\delta$ will map to the executed value obtained from our program. In addition we also need to justify the program structure generated from an graph structure is correct. To do so we extend figure \ref{fig:correctness_graph1} iteratively to obtain figure \ref{fig:correctness_graph2} to incorporate transitions.
...
The output atom generated refers to PORTOUT, which is mapped on a hardware level to the PLC hardware implementer's designation of an appropriate output port. This is done to allow the hardware manufacturer a bit of flexability. In our implementation PORTOUT can be assigned any 8-bit value. Any values larger than 8-bit will be trucated. In our example shown in figure \ref{fig:correctness_atom_output} we can see that the line observed in the diagram is ``PORTOUT := 0xF2'' and the final output is ``PORTOUT = 0xF2''. The expressions differ by the syntax ``:='' is replaced by ``='' noting this we can assign any expression as long as it follows the syntax for \plccharts, and it can be evaluated to a value.
...
The input atom generates final code that is slightly different that what is shown on the digram. We can see in figure \ref{fig:correctness_atom_input} that the diagram clearly shows ``int var := PORTIN''. The final compiled code for the atom is ``var = PORTIN'' the differences are ``int'' is not stated in the final compiled code the type information is used in the construction shown in section \ref{structure}. In addition similarly ``:='' is replaced by ``='' in the compiled code. The left hand side must be a valid variable name as specified in the syntax of \plccharts. The variable can be any type however PORTIN will always be an 8-bit integer any variable on the left hand side that is not integer would have PORTIN automatically casted to the appropriate type.
...
The store block is used for basic assignments. Similar to the input and output examples above the store block generates code that slightly differs from the diagram. First ``:='' is also replaced with ``='' as before where it occurs. In the left example in figure \ref{fig:correctness_atom_store} we see a store with one assign statement and to the right of figure \ref{fig:correctness_atom_store} we see that store blocks can contain multiple atomic assign statement. 

Looking at the left diagram first we note that the assign statement is ``int a := 0'' and there is an additional line ``<double click here> <name> := <value>'' first we observe that the last line is omitted entirely the last line is a place holder to allow the user to add more lines. Next we note that the substituttion ``:='' to ``='' is made. Finally we note that the type ``int'' is not included in the final generated code ``a = 0''. Similar to the input and output atoms previous the type is used in construction only and is not part of the atomic assigns.

Looking at the right diagram we see that the last line is also omitted int he final atom. However each line off the assign are their own atomic assign statements. We see that ``int a := a +1'' becomes ``a = a + 1'' and ``int b := a'' becomes ``b = a''. Once again the type does not show up in the atomic assigns as it's use to declare and initialize the variables in the final construction (more details are in the constructor section \ref{structure}). We note that we can create equivalent store atoms by having two single line store atoms on the left diagram. Thus, it is not difficult to see that the right diagram is just a visual grouping of several atomic assigns for visual simplicity when drawn. Being able to group assigns together into one atomic diagram removes the need to have several atomic assign blocks which ends up being visual noise.
