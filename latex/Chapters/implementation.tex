...
...
08 \textbf{if (\emph{condition}) goto \emph(LABEL)
     ...
09 goto EOF;
\end{lstlisting}

Again we start the store block similar tot he other blocks with identical structure until line 04. Line 05-07 are the code conversions for the store block. The store block converts each line in the diagram of the form ``type variable_name := expression'' into ``variable_name = expression'' as seen on line 05. In addition if there is more than one line they are then placed one line at a time in the subsequent lines. The expression can be any valid mathematical ``C'' style expression, and functions are not supported.